<template>
  <div class="mainWrapper">
    <div class="tanksWorld" id="tanksWorld">
      ...
      <div class="signOut gameopaction" @click="quitGame">
        退出
      </div>
    ...
    <div>
      <i v-for="(item, idx) in tankList" class="iconfont icon-kejichangcheng-tankepingtai"
        :style="{ left: item.tkX + 'px', top: item.tkY + 'px',  transform: getTransformStyle(item.tkOrient)}" :title="item.tkName" @dblclick="selectThisTank(item)"
...
export default {
  data() {
    return {
      ...
      direction: "top" //当前坦克方向
    }
  },
  methods: {
    getTransformStyle(direction) {
      if (direction == "top") {
        return "rotate(0)";
    ...
    selectThisTank(tankItem) {
      ...
      this.selectTank = tankItem;
      document.getElementById(this.nowTankDomId).style.color = "red";
      document.getElementById(this.nowTankDomId).style.fontWeight = "900";
      this.direction = tankItem.tkOrient; //获取当前坦克方向
      const rotate = this.getTkRotate(direction);
      document.getElementById("ammo").style.transform = rotate; //更新子弹方向
    },
    getTkRotate(direction) {
      let rotate = "rotate(0)"
      if (direction === "top") {
        rotate = "rotate(0)"
      } else if (direction === "bottom") {
    ...
    setTankDirection() {
      this.tankList.map((elem) => {
        const rotate = getTkRotate(elem.tkOrient)
        document.getElementById("T-" + elem.tkId).style.transform = rotate;
      })
    },
    getTanksList() {
    ...
    clickNewGame() {
      ...
      this.tankForm.tkOrient = "top" //设置坦克朝向默认值为top
  ...
  mounted() {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (event.keyCode == 87 || event.keyCode == 38) {
        ...
        if (this.direction == "top" || this.direction === "bottom") {
          ...
          document.getElementById("ammo").style.top = top + "px";
        ...
        let timer = setInterval(() => {
          ...
          if (this.direction == "top" || this.direction == "bottom") {
            document.getElementById("ammo").style.top = tTop + "px";
            document.getElementById("ammo").style.left = document.getElementById("ammo").style.left - 2
          } else {
            document.getElementById("ammo").style.top = tTop + 5 + "px";
            document.getElementById("ammo").style.left = tLeft + "px";
          }
          count++;
          if (count > 40) {
    ...
    document.addEventListener("keyup", (event) => {
      ...
      if (event.keyCode == 87 || event.keyCode == 38) {
        ...
        //右
        console.log("r");
        console.log(this.nowcontrolTankId);
        var sendObj = {
          tkId: this.nowcontrolTankId,
          filesId: this.nowGameId,
          tkName: this.selectTank.TKname,
